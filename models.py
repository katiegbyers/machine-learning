...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.w4 = nn.Parameter(50, 200)
        self.b4 = nn.Parameter(1,40)
        self.w5 = nn.Parameter(50,20)
    ...
    def run(self, xs):
        ...
        relu2 = nn.ReLU(f1_of_x)
        w3x = nn.Linear(relu2,w3)
        h1 = nn.AddBias(w3x, b3)

        for i in range(1,len(xs)):
            w1x = nn.Add(nn.Linear(xs[i],w1), nn.Linear(h1,w4))
            w1x_plus_b1 = nn.AddBias(w1x,b1)
            relu = nn.ReLU(w1x_plus_b1)
            w2x = nn.Linear(relu,w2)
            f1_of_x = nn.AddBias(w2x,b2)
            ...
            w3x = nn.Linear(relu2,w3)
            h1 = nn.AddBias(w3x, b3)
        return nn.AddBias(nn.Linear(nn.ReLU(nn.AddBias(nn.Linear(h1,w5),b5)),w6),b6)
        ...
        #h1 = nn.Linear(xs[0],w2)
       # for i in range(1,len(xs)):
       #     h2 = nn.Add(nn.Linear(xs[i],w2), h1)
       # return h1
...
#        w1x = nn.Linear(xs[0],w1)
    ...
    #    h1 = nn.AddBias(w3x, b3)
    #    for i in range(1,len(xs)):
    #        w1x = nn.Linear(xs[i],w1)
    #        w1x_plus_b1 = nn.AddBias(w1x,b1)
    #        relu = nn.ReLU(w1x_plus_b1)
    #        w2x = nn.Linear(relu,w2)
    #        f1_of_x = nn.AddBias(w2x,b2)
    #        relu2 = nn.ReLU(f1_of_x)
    #        w3x = nn.Linear(relu2,w4)
      #      h2 = nn.AddBias(w3x, b3)
    ...
    def train(self, dataset):
        ...
        while dataset.get_validation_accuracy() < .9:
            for x, y in dataset.iterate_once(batch_size):
                loss = self.get_loss(x,y)
                grad_wrt_w1, grad_wrt_b1, grad_wrt_w2, grad_wrt_b2, grad_wrt_w3, grad_wrt_b3, grad_wrt_w4, grad_wrt_w5, grad_wrt_b5, grad_wrt_w6, grad_wrt_b6 = nn.gradients(loss, [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3, self.w4, self. w5, self.b5, self.w6, self.b6])
                self.w1.update(grad_wrt_w1, multiplier)
                self.b1.update(grad_wrt_b1, multiplier)
                self.w2.update(grad_wrt_w2, multiplier)
                self.b2.update(grad_wrt_b2, multiplier)
